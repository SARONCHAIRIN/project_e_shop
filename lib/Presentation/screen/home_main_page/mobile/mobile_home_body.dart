...
class MobileHomeBody extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<MobileHomeBody> createState() => _MobileHomeBodyState();
}

class _MobileHomeBodyState extends ConsumerState<MobileHomeBody> {
  ...
  void initState() {
    super.initState();
    // 1. Fetch categories and sync selected category from args on start
    _loadCategories();
  }
  ...
  Future<void> _loadCategories() async {
    try {
      ...
      setState(() {
        categories = result;
        // Keep selected category in sync with route/view arguments if passed
        selectedCategory = widget.args.selectedCategory;
        isLoadingCategory = false; // 2. Stop shimmer loading state
      });
    } catch (e, stack) {
        ...
        SliverToBoxAdapter(
          child: Column(
            children: [
                  ...
                  Navigator.push(
                    ...
                    MaterialPageRoute(
                      builder: (context) => SeeAllCategory(
                        repository:
                            widget.args.authRepository ?? widget.authRepository,
                      ),
                    ),
                  );
                }),
              ...
              // 3. This will now properly toggle once API returns data
